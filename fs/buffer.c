 ...
 * 因此就需要开关中断指令（cli-sti）序列来检测等待调用返回。但需要非常地快
 * (希望是这样).
 */
...
repeat:
    ...
    do
    {
        ...
        /**
         * 如果缓冲头指针bh为空，或者tmp所指缓冲头的标志(修改、锁定)权重小于bh头标志的权重，
         * 则让bh指向该tmp缓冲区头。如果该tmp缓冲区头表明缓冲区既没有修改也没有锁定标志置位，
     ...
     * 加入进高速缓冲中，所以要对此进行检查.
     * 在高速缓冲hash表中检查指定设备和块的缓冲区是否已经被加入进去。如果是的话，
     * 就再次重复上述过程.
     */
...
struct buffer_head *bread(int dev, int block)
{
    ...
    /* 在高速缓冲中申请一块缓冲区。如果返回值是NULL指针，表示内核出错，死机. */
    ...
    /* 否则表明读设备操作失败，释放该缓冲区，返回NULL指针，退出. */
    brelse(bh);
...
/*
 * bread_page一次读四个缓冲块内容读到内存指定的地址。它是一个完整的函数，
...
struct buffer_head *breada(int dev, int first, ...)
{
    ...
    /* 可变参数表中所有参数处理完毕。等待第1个缓冲区解锁(如果已被上锁). */
    va_end(args);
    wait_on_buffer(bh);
...
void buffer_init(long buffer_end)
{
    ...
    /**
     * 如果缓冲区高端等于1Mb，则由于从640KB-1MB被显示内存和BIOS占用，因此实际可用缓冲区
     * 内存高端应该是640KB。否则内存高端一定大于1MB.
     */
    if (buffer_end == 1 << 20)
        b = (void *)(640 * 1024);
    else
     ...
     * 操作的过程是从缓冲区高端开始划分1K大小的缓冲块，与此同时在缓冲区低端建立描述
     * 该缓冲块的结构buffer_head，并将这些buffer_head组成双向链表。
     * h是指向缓冲头结构的指针，而h+1是指向内存地址连续的下一个缓冲头地址，也可以说
     * 是指向h缓冲头的末端外。为了保证有足够长度的内存来存储一个缓冲头结构，需要b所
     * 指向的内存块地址>=h缓冲头的末端，也即要>=h+1.
     */
    while ((b -= BLOCK_SIZE) >= ((void *)(h + 1)))
    {
        ...
        h->b_data   = (char *)b;    /* 指向对应缓冲区数据块(1024字节). */
    ...
    /* 初始化hash表(哈希表、散列表)，置表中所有的指针为NULL. */
    for (i = 0; i < NR_HASH; i++)
        hash_table[i] = NULL;
}
