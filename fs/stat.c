...
/**
 * cp_stat - 复制文件状态信息。
 * 参数inode是文件对应的inode，statbuf是stat文件状态结构指针，
 * 用于存放取得的状态信息。
 */
static void cp_stat(struct m_inode *inode, struct stat *statbuf)
{
    ...
    tmp.st_ino   = inode->i_num;        /* 文件inode号. */
    ...
    tmp.st_rdev  = inode->i_zone[0];    /* 设备号. */
...
int sys_stat(char *filename, struct stat *statbuf)
{
    ...
    /* 首先根据文件名找出对应的inode，若出错则返回错误码. */
    if (!(inode = namei(filename)))
        return -ENOENT;

    /* 将inode上的文件状态信息复制到用户缓冲区中，并释放该inode. */
...
int sys_fstat(unsigned int fd, struct stat *statbuf)
{
     ...
     * 结构指针为空，或者对应文件结构的inode字段为空，则出错，返回出错码并退出.
     */
    if (fd >= NR_OPEN || !(f = current->filp[fd]) || !(inode = f->f_inode))
        return -EBADF;

    /* 将inode上的文件状态信息复制到用户缓冲区中. */
    cp_stat(inode, statbuf);
